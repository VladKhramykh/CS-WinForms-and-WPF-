        ...
        private void payment(object sender, EventArgs e)
        {
            if (choiceAdress.SelectedItem != null)
            {
                ...
                try
                {
                    ...
                    int choice = Adress.indexOf(choiceAdress.Text);
                    if (choice != -1)
                    {
                        Adress.getAdress(choice).addApartment(sumFootage, number, kitchen, toilet, bath, balcon, rooms);
                        JSONController.SaveJsonFormat(Adress.getAdresses(), "Adresses.json");
                    }
                    else
                    ...
                    showBox.Items.Add(Adress.getAdress(choice));
